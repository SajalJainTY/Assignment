...
public class PendingActions_page {
	...
	public class PendingActions_Page {
		 ...
		 * @author Vivek Dogra
		 */
		public synchronized void validatePendingActionpage(String expectedPendingActionsPageText,String expectedPendingActionsPageUrl,String 

	expectedPendingActionsPageTitle) {
			try {
//				WebActionUtil.validateisElementDisplayed(txtPendingAction, "Pending Actions Text",
//						"Pending Action Page is displayed", "Pending Action Page is not displayed", "blue");
						String actualClaimPageText =driver.findElement(By.xpath("//div[@class='pull-left 

	topHeader']/descendant::li[2]")).getText();
					    String actualUrl =driver.getCurrentUrl();
					    String actualTitle =driver.getTitle();
					    
					    if(actualClaimPageText.equals(expectedPendingActionsPageText) && actualUrl.equals

	(expectedPendingActionsPageUrl) && actualTitle.equals(expectedPendingActionsPageTitle))
					    {
					    	WebActionUtil.validationinfo("Pending Action Page is displayed, url ="+driver.getCurrentUrl()+ ", 

	title=" +actualTitle,"blue");
					    }
		...
		/**
		 * Description: Click on Pending Approval Button
		 * 
		 * @author Vivek Dogra
		 */
		...
		public synchronized void clickOnApproveButton() {
			try {
				WebActionUtil.validateisElementDisplayed(btnApprove, "Approve button", "Approve button is displayed",
						"Approve button is not displayed", "blue");
		...
		public synchronized void clickOnSubmitReferBackButton() {
			...
			try {
				...
				try {
					new WebDriverWait(driver, 6).until(ExpectedConditions.invisibilityOf(cancelToastmsg));
				} catch (Exception e) {
		...
		public synchronized void clickOnYesButton() {
			...
			Thread.sleep(2000l);
				/*
				 * WebActionUtil.validateisElementDisplayed(btnYes, "Pop up Yes button",
				 * "Pop up Yes button is displayed","Pop up Yes button is not displayed",
				 * "blue");
				 */
				WebActionUtil.validatetext("Are you sure you want to proceed?", txtApproveWarningMessage,
						"Are you sure you want to proceed pop up", "Are you sure you want to proceed pop up is 
	...
	displayed",
		...
		if (ApproveToastmsg.isDisplayed()) {
					try {
						new WebDriverWait(driver, 7).until(ExpectedConditions.invisibilityOf(ApproveToastmsg));
					} catch (Exception e) {
		...
		public synchronized void clickOnBulkApprovelpopupYesButton() {
			try {
				...
				WebActionUtil.clickOnElement(btnBulkApprovalPopUpYes, "Bulk Approvel pop up Yes button",
		...
		public synchronized void clkPaticularCheckbox(String claimid) {
			try {
				...
				WebActionUtil.clickOnElement(getCheckbox(claimid), "Particular claim check box",
						"Unable to click on particular claim check box");
			} catch (Exception e) {
				WebActionUtil.error(e.getMessage());
				...
				if (expectedList.size() == actualList.size()) {
					for (int i = 0; i < expectedList.size(); i++) {
						if (!Arrays.equals(expectedList.get(i), actualList.get(i))) {
							flag = true;
							if (flag) {
								WebActionUtil.info(Arrays.toString(expectedList.get(i)) + " not present in 

	History pop up");
								WebActionUtil.fail(
										"Expected history is not matching with the actual 

	history in the application");
								Assert.fail("Expected history is not matching with the actual history in the 
	...
	application");
		...
		public synchronized void clkClaimID(String claimID) {
			try {
				...
				WebActionUtil.clickOnElement(expectedClaimID, "Claim ID text", "Unable to click on Claim ID text");
				WebActionUtil.waitForAngularPageload();
			} catch (Exception e) {
				WebActionUtil.error(e.getMessage());
				WebActionUtil.fail("Unable to click on Claim ID text");
		...
		public synchronized void clickApproveButtonunderClaimpopup() {
			try {
				...
				WebActionUtil.waitForElement(txtApproveWarningMessage, "Are you sure you want to proceed pop up");
				WebActionUtil.validatetext("Are you sure you want to proceed?", txtApproveWarningMessage,
						"Are you sure you want to proceed pop up", "Are you sure you want to proceed pop up is 
	...
	displayed",
		...
		public synchronized void validatePendingActionsData() {
			...
			if (isHeadersPresent && isDataPresent) {
				WebActionUtil.validationinfo("List of all claims that need approval is displayed", "blue");
				WebActionUtil.info("List of all claims that need approval is displayed");
			} else {
	...
	/**
		 * Description: Method to validate Claim ID removed from pending action page
		 * 
		 * @author Suganthini, vikas
		 * @param sapID
		 * @param claimId
		 */
		public synchronized void validateClaimID(String sapID, String claimId) {
			List<WebElement> expectedClaimID = null;
			try {
				expectedClaimID = driver.findElements(By.xpath("//a[text()='" + sapID + "']"));
				String claimRemoveStatus = expectedClaimID.size() == 0 ? claimId + " is removed"
						: claimId + " is not removed";
				WebActionUtil.info("Claim ID : " + claimRemoveStatus);
				if (expectedClaimID.size() <= 0) {
					WebActionUtil.info("Claim is removed from Pending action page and moved to next level for approval");
					WebActionUtil.pass("Claim is removed from Pending action page and moved to next level for approval");
				} else {
					WebActionUtil.error(
							"Claim is not removed from Pending action page and not moved to next level for 

	approval");
					WebActionUtil
							.fail("Claim is not removed from Pending action page and not moved to next level for 
	...
	approval");
				...
				WebActionUtil.error(e.getMessage());
				WebActionUtil
						.fail("Claim is not removed from Pending action page and not moved to next level for approval");
				Assert.fail("Claim is not removed from Pending action page and not moved to next level for approval");
			}
